...
urls_group_0 = [url for idx, url in enumerate(product_urls) if idx % 3 == 0]
urls_group_1 = [url for idx, url in enumerate(product_urls) if idx % 3 == 1]
urls_group_2 = [url for idx, url in enumerate(product_urls) if idx % 3 == 2]
...
def crawl_group(group_urls, group_name):
    for url in tqdm(group_urls, desc=f'Crawling {group_name}'):
        ...
        if item_id and shop_id:
            ...
            comments = crawl_comments(item_id, shop_id,max_reviews=100000)
            append_to_csv(pd.DataFrame(comments), f'{group_name}_comments.csv')

with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
    futures = [
        executor.submit(crawl_group, urls_group_0, 'group_0'),
        executor.submit(crawl_group, urls_group_1, 'group_1'),
        executor.submit(crawl_group, urls_group_2, 'group_2'),
    ]
